      ...
      <div className='lg:pt-20 pt-12 px-4 flex flex-col items-center text-center'>
        <div className='lg:space-y-8 space-y-8'>
          ...
          <p className='text-gray-600 mb-8 text-base lg:text-lg font-aeonik font-normal md:max-w-2xl lg:max-w-4xl mx-auto'>
            Explore a selection of my favorite projects that showcase my expertise in branding, graphic design, animation, and video editing. Each project is a testament to my dedication to creativity and excellence. Browse my solutions across
            industries.
          </p>
        </div>
        ...
        <div className='md:flex gap-4 py-8 mt-10 grid grid-cols-2'>
          {navigationData.map((item) => (
            <Popover
              key={item.key}
              overlayClassName="max-w-['!calc(100vw - 32px)']"
              content={
                <>
                  <div className={`md:grid-cols-4 grid-cols-3 p-2 md:py-5 grid grid-rows-2 gap-x-5 md:gap-y-1`} role='menu'>
                    {item.items.map((item, index) => (
                      <button key={index} className='flex items-center w-full md:px-4 py-2 text-sm text-gray-700 hover:text-primary group' role='menuitem'>
