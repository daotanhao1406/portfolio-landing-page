...
export function MobileNav({ items, isOpen, onClose, onToggle }: MobileNavProps) {
  ...
  return (
    <div className='lg:hidden'>
      ...
      <div ref={navRef} className={cn('fixed inset-y-0 bg-[#F1DEBB] transform transition-transform duration-300 ease-in-out z-50 px-2', 'w-full md:w-1/2', 'right-0', isOpen ? 'translate-x-0' : 'translate-x-full')}>
        <div className={cn('fixed inset-0 bg-[#F1DEBB] transition-opacity duration-300', 'md:block hidden', isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none')} onClick={onClose} />
